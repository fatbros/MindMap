$(function(){
    ...
    function init(){
        ...
        socket.on('login_message',function(data){
            var selector_name = $('<p>',{id: data.name});
            selector_name.text(data.name + ' さんがログインしました');
            $('#login_user').append(selector_name);
        });
        socket.on('logout_message',function(data){
            var selector_name = $('#login_user #' + data.name);
            selector_name.text(data.name + ' さんがログアウトしました');
    ...
    Canvas.prototype.stageEvent = function(){
        ...
        function mouseMove(e){
            ...
            that.emit_draw_coord.push(push_local);
            //座標の入れ替え
